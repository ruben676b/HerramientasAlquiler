...
function initDatabase() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS ETIQUETA_CLIENTE (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      nombre TEXT NOT NULL UNIQUE,
      color TEXT NOT NULL DEFAULT '160',
      activo INTEGER NOT NULL DEFAULT 1 CHECK (activo IN (0, 1))
    );

    CREATE TABLE IF NOT EXISTS CLIENTE_ETIQUETA (
      id_cliente INTEGER NOT NULL REFERENCES CLIENTE(id) ON DELETE CASCADE,
      id_etiqueta INTEGER NOT NULL REFERENCES ETIQUETA_CLIENTE(id) ON DELETE CASCADE,
      PRIMARY KEY (id_cliente, id_etiqueta)
    );
