            ...
            {modoItem === 'individual' ? (
              <div className="flex gap-2 items-end">
                      ...
                      {herramientasFiltradas.map(h => (
                        <div key={h.id} className="w-full flex items-center gap-1 pl-3 pr-2 py-1.5 text-xs hover:bg-[var(--surface)]">
                          <button
                            ...
                            className="flex-1 min-w-0 text-left flex justify-between items-center gap-2"
                            style={{ color: 'var(--ink)' }}>
                            <span className="truncate">
                              <span className="font-mono font-bold" style={{ color: 'var(--primary)' }}>{h.id}</span> — {h.nombre}
                            </span>
                            <span className="font-mono shrink-0" style={{ color: 'var(--muted)' }}>S/ {h.precio_dia.toFixed(2)}/día</span>
                          </button>
                          <OjoPreview tipo="herramienta" id={h.id} titulo={h.id + ' — ' + h.nombre} />
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <OjoPreview
                  ...
                  ruta={herrSel?.imagen_path}
                  titulo={herrSel ? herrSel.id + ' — ' + herrSel.nombre : undefined}
                  variante="boton"
                  lado="izquierda"
                />
                <Button
              ...
              <div className="flex gap-2 items-end flex-wrap">
                ...
                <OjoPreview
                  ...
                  ruta={granelSel?.imagen_path}
                  titulo={granelSel ? granelSel.nombre + ' (' + granelSel.condicion + ')' : undefined}
                  variante="boton"
                  lado="izquierda"
                />
                <Button
                          ...
                          <td className="px-4 py-3" style={{ color: 'var(--ink)' }}>
                            <div className="flex items-center gap-2">
                              ...
                              {(item.tipo === 'individual' || item.tipo === 'granel') && (
                                <OjoPreview
                                  tipo={item.tipo === 'individual' ? 'herramienta' : 'granel'}
                                  id={item.tipo === 'individual' ? item.id_herramienta : item.id_item_granel}
                                  titulo={item.nombre}
                                />
                              )}
                            </div>
